def call(applicationName){
  ...
  stage('Download Artifact'){
    ...
    sh "curl -Lo artifact.tgz https://artifacts.comapny.com/search?name=${applicationName}&version=latest"
    artifactTimer.stop()
  }

  stage('Docker Image') {
    def dockerBuild = metricsFactory.timer("image", "dockerBuild")
    sh "docker build -t docker.company.com/${applicationName}:latest . "
    dockerBuild.stop()

    def dockerPush = metricsFactory.timer("image", "dockerPush")
    sh "docker push docker.company.com/${applicationName}:latest"
    dockerPush.stop()
  }
